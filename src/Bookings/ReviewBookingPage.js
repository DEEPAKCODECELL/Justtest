import { View, Text, TouchableOpacity, TextInput, Animated, Modal, BackHandler, SafeAreaView, ActivityIndicator, Alert, Linking } from "react-native";
...
import { applyPromoCode } from "../redux/slices/bookingSlice";
...
const ReviewBookingPage = () => {
  ...
  const handleapplypoints = () => {
    ...
    if (pointsUsed != 0) {
      console.log('inde')
      setShowFinalPrice(((Bookingdata.finalPrice) / 100) - (((promoCodeDiscount / 100))));
      SetPointsUsed(() => 0);
      return;
    }
    ...
    if ((userPoints) > maxPointsAllowed) {
  ...
  const handleNavigationStateChange = async (navState) => {
    console.log("🔍 WebView navigation detected:", navState.url);

    if (navState.url.includes('success')) {
      console.log("✅ Success URL detected:", navState.url);

      const queryParams = new URLSearchParams(navState.url.split("?")[1]);
      const transactionId = queryParams.get("transactionId");

      console.log("🆔 Extracted Transaction ID:", transactionId);

      if (transactionId) {
        console.log("🚀 Navigating to BookingSuccess with transactionId:", transactionId);
        navigation.navigate("BookingSuccess", { transactionId });
      } else {
        console.log("❌ Transaction ID not found in success URL!");
      }
    } else if (navState.url.includes('failure')) {
      console.log("❌ Failure URL detected:", navState.url);
      setPaymentUrl(null); // Close WebView
    } else if (navState.url.startsWith("mynewapp://")) {
      console.log("🚀 Detected Custom URL Scheme:", navState.url);

      // **Instead of WebView trying to open it, handle it manually**
      try {
        const transactionId = navState.url.split("transactionId=")[1];

        if (transactionId) {
          console.log("🔄 Navigating inside the app with transaction ID:", transactionId);
          navigation.navigate("VerifyPaymentPage", { transactionId, BookingId: Bookingdata.BookingId, locationdata, modeOfPayment, pointsUsed, showFinalPrice });
        } else {
          console.log("❌ No transaction ID found in custom URL");
        }
      } catch (error) {
        console.error("⚠️ Error handling custom URL:", error);
      }
    } else {
      console.log("ℹ️ Other URL detected, no action taken:", navState.url);
    }
  };

  const handleBooking = async () => {
    if (modeOfPayment == "cash") {
      console.log("Navigate With All Those Data",Bookingdata.BookingId, locationdata, modeOfPayment, pointsUsed, showFinalPrice)
      navigation.navigate("VerifyPaymentPage", { BookingId: Bookingdata.BookingId, locationdata, modeOfPayment, pointsUsed, showFinalPrice});
      return;
    }
    console.log("handling pament");
