...
const Header = ({title = 'Your Address', showBackButton = true}) => {
  ...
  return (
    <View
      style={tw`flex-row items-center p-5 bg-white border-b border-gray-200`}>
      {showBackButton && (
        <TouchableOpacity
...
const SearchBox = ({placeholder = 'Add a new address', onSelect}) => {
  ...
  return (
    <View style={tw`relative w-full p-3`}>
      ...
      />
      {loading && (
        <ActivityIndicator size="small" color="gray" style={tw`mt-2`} />
      )}
      ...
      {searchQuery &&
        ...
        suggestions.length > 0 && (
          <View
            style={tw`absolute top-full left-0 w-full bg-white border border-gray-300 rounded-lg shadow-lg z-50`}>
            <FlatList
              data={suggestions}
...
const SavedLocation = ({location, onClick}) => {
  ...
  return (
    <TouchableOpacity
      style={tw`flex-row items-start p-4 rounded-lg bg-gray-50 mb-2`}
      onPress={() => onClick?.(location)}>
